...
function getTopAnimes(p) {
  ...
  fetch('https://anime-tv-v3-api-n65d.vercel.app/topanime?type=bypopularity&page=' + String(page), { method: 'GET' })
    .then(response => {
      if (response.ok) {
...
function animeInfo(mal_id) {
  ...
  fetch('https://anime-tv-v3-api-n65d.vercel.app/anime/' + (mal_id), { method: 'GET' }).then(response => {
...
function showChars(mal_id){
  ...
  if(sessionStorage.getItem(`${mal_id}Chars`) == null){
    fetch('https://anime-tv-v3-api-n65d.vercel.app/anime/' + (mal_id) +'/characters', { method: 'GET' }
    ).then(response => {
      if (response.ok) { return response.json();}
      throw new Error('Request failed!');
    }).then(jsonResponse => {
